...
- **注意事项：** **解析成本最高**。Canal 格式比普通 JSON 胖出一大圈，如果每秒几十万 QPS，解析它会消耗掉大量的 CPU。
...
`canal-json` 包含了 `old`、`data`、`type` 等大量元数据。在 **高吞吐（每秒 10w+）** 场景下，解析 `canal-json` 的 CPU 开销比普通 `json` 高出约 $20% ~ 30% $。
